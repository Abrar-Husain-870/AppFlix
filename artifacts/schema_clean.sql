-- ============================================================
-- APPFLIX — CLEAN DATABASE SCHEMA
-- Run this in Supabase SQL Editor (Dashboard → SQL Editor)
--
-- CHANGES vs the original schema.sql:
--   1. Removed orphaned trigger `set_comments_updated_at` (targets
--      public.comments which is NOT in the MVP schema — comments were
--      intentionally excluded). The original file had a live trigger
--      referencing a non-existent table which would throw an error.
--   2. Removed orphaned RLS policy on `project_status_history`
--      (that table was also intentionally removed from MVP schema).
--   3. Updated category seed data from DeenList Islamic-app categories
--      to AppFlix-relevant university student project categories.
--   All other schema, triggers, RLS policies, and storage config are
--   identical to the frozen original schema.sql.
-- ============================================================
...
-- Seed data — university student project categories (replaced DeenList Islamic-app categories)
INSERT INTO public.categories (name, slug, icon, sort_order) VALUES
  ('Web Apps',            'web-apps',         '🌐', 1),
  ('Mobile Apps',         'mobile-apps',      '📱', 2),
  ('AI & Machine Learning','ai-ml',           '🤖', 3),
  ('Tools & Productivity','tools',            '⚡', 4),
  ('Games',               'games',            '🎮', 5),
  ('Open Source',         'open-source',      '🔓', 6),
  ('Data & Analytics',    'data-analytics',   '📊', 7),
  ('Design & Creative',   'design-creative',  '🎨', 8),
  ('Hackathon Projects',  'hackathon',        '🏆', 9),
  ('Class Projects',      'class-projects',   '🎓', 10),
  ('Browser Extensions',  'browser-extensions','🧩', 11),
  ('Other',               'other',            '📦', 99);
...
-- Note: project_status_history was intentionally removed from schema.
-- Status change tracking is handled in application code (Server Actions).
...
-- NOTE: set_comments_updated_at trigger REMOVED.
-- The comments table was intentionally excluded from the MVP schema.
-- Uncomment and add the comments table when the feature is implemented.
...
-- NOTE: project_status_history RLS policy REMOVED.
-- The project_status_history table was intentionally removed from the MVP schema.
-- Status change tracking (who approved, when) is recorded in projects.approved_at
-- and projects.rejection_reason. A full audit log can be added as a separate
-- application-level insert in the Server Action itself.
